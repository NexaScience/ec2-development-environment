...
```bash
~/scripts/start-claude-sessions.sh <session-name>
# 例: ~/scripts/start-claude-sessions.sh my-task
```
...
4. 環境変数ファイル（`.claude-env`）の作成
5. アプリリポジトリのクローン（`/home/ubuntu/app`）
6. `devcontainer up` でDevcontainerをビルド・起動
7. スクリプト群をホストで生成し `docker cp` でコンテナ内にコピー
8. Slack起動通知の送信
...
- `user_data` はインスタンスの **初回起動時のみ** 実行される。設定変更後は `terraform apply` でインスタンスを再作成する必要がある
